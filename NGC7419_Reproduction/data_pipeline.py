...
def fetch_and_clean_data(ra_str: str, dec_str: str, radius_arcmin: float) -> pd.DataFrame:
    """
    Fetches and filters Gaia DR3 data using ADQL based on the paper's constraints.
    """
    # Convert string coordinates to degrees
    coord = SkyCoord(f"{ra_str} {dec_str}", unit=(u.hourangle, u.deg))
    radius_deg = radius_arcmin / 60.0
    
    # ADQL query enforcing the exact constraints from Chakraborty et al. 2025
    query = f"""
    ...
    WHERE 1=CONTAINS(POINT('ICRS', ra, dec), CIRCLE('ICRS', {coord.ra.deg}, {coord.dec.deg}, {radius_deg}))
      AND parallax > 0.1 AND parallax < 0.8
      AND ruwe < 1.4
