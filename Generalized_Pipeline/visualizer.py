...
def plot_results(full_df: pd.DataFrame, members_df: pd.DataFrame):
    ...
    # Plot 1: Proper Motion Vector Point Diagram
    ax1.scatter(full_df['pmra'], full_df['pmdec'], s=5, color='grey', alpha=0.3, label='All Sources')
    ...
    ax2.scatter(full_df['bp_rp'], full_df['Gmag'], s=5, color='grey', alpha=0.3, label='All Sources')
    ax2.scatter(members_df['bp_rp'], members_df['Gmag'], s=15, color='green', alpha=0.9, label='Predicted Members')
    
    # In astronomy, brighter magnitudes have lower numerical values
    ax2.invert_yaxis()
